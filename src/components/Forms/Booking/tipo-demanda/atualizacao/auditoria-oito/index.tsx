...
interface FormAtualizacaoBookingAuditoriaOitoProps {
...
export function FormAtualizacaoBookingAuditoriaOito({ params }: FormAtualizacaoBookingAuditoriaOitoProps) {
  const pk = params.id;
  ...
  const validarCamposAuditados = () => {
    ...
    const chavesCamposNaoAuditados = Object.keys(camposAuditaveis).filter((key) => camposAuditaveis[key as keyof CamposAuditadosCadastroJuridico] === undefined);

    const nomesCamposNaoAuditados = chavesCamposNaoAuditados.map((key) => dicionarioCamposAuditados[key as keyof typeof dicionarioCamposAuditados]);
  ...
  const AlertaCampoAuditado = ({ value }: { value: boolean | undefined }) => {
    if (value === undefined) {
      return (
        <hr 
          className="bg-orange-300 dark:bg-orange-400 h-2 rounded-full"
        />
      )
    }
