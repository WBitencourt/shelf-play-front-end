...
import { enviarDesdobramentoProcesso } from '@/actions/backend-for-front-end/cliente/return';
...
interface FormProconBookingAuditoriaOitoProps {
...
export function FormProconBookingAuditoriaOito({ params }: FormProconBookingAuditoriaOitoProps) {
  const pk = params.id;
  ...
  const defaultCamposAuditados: CamposAuditadosProcon = {
  ...
  const validarCamposAuditados = () => {
    const camposAuditaveis = values?.campos_auditados as CamposAuditadosProcon;
    ...
    const chavesCamposNaoAuditados = Object.keys(camposAuditaveis).filter(
      (key) => camposAuditaveis[key as keyof CamposAuditadosProcon] === undefined
    );
    ...
    toast.custom.warning('Há campos não auditados. Por favor, revise os campos e tente novamente.', {
      ...
      //     onClick={() => copyToClipBoard(nomesCamposNaoAuditados.join(', '))}
      //   >
      //     <span className="text-black dark:text-white">Copiar</span>
      //   </Button>
      // ),
      description: (
        <div>
                ...
                <div className="space-y-4 bg-blue-50 dark:bg-blue-950/50 p-3 rounded-md mb-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="group flex gap-2 items-center">
                      ...
                      <div className="ml-2 flex gap-2">
                        <ThumbsUp
                          data-active={values?.campos_auditados?.data_defesa === true}
                          className="w-4 h-4 hover:scale-125 cursor-pointer text-gray-400 data-[active=true]:text-green-500 hover:text-green-500"
                          onClick={() => formBag.setValue('campos_auditados.data_defesa', true)}
                        />
                        <ThumbsDown
                          data-inactive={values?.campos_auditados?.data_defesa === false}
                          className="w-4 h-4 hover:scale-125 cursor-pointer text-gray-400 data-[inactive=true]:text-red-500 hover:text-red-500"
                          onClick={() => formBag.setValue('campos_auditados.data_defesa', false)}
                        />
                      </div>
                    </div>
                    <div className="flex flex-col gap-2">
                      <AlertaCampoAuditado value={values?.campos_auditados?.data_defesa} />
                    </div>
                  </div>
                ...
                <div className="space-y-4 bg-green-50 dark:bg-green-950/50 p-3 rounded-md mb-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Select {...formBag.register('uf')} value={values.uf} onValueChange={handleUfChange}>
                        <SelectTrigger>
