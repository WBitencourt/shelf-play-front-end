...
import { DadosProcesso } from '@/actionsV2/backend/demanda/service';
import { convertBRLCurrencyToNumber, formatNumberBRLCurrency } from '@/utils/String';

import {
  DadosDemandaCadastroJuridico,
  OnUploadErrorProps,
  FormFile,
  OnDropAccepted,
  OnDropRejected,
...
interface CadastroJuridicoReturnParams {
  ...
  esteira: string;
  esteiraTipo: string;
  dadosIniciais: DadosDemandaCadastroJuridico;
}

interface FormCadastroJuridicoSumUpEsteiraOitoProps {
...
export function FormCadastroJuridicoSumUpEsteiraOito({ params }: FormCadastroJuridicoSumUpEsteiraOitoProps) {
  const pk = params.id;
  ...
  const formBag = useForm<DadosDemandaCadastroJuridico>({
    defaultValues: {
      observacao: [],
      tipificacao: '',
      uf: '',
  ...
  const [dadosConsultaProcesso, setDadosConsultaProcesso] = useState<ProcessoInfo[]>([]);

  const dateNow = useMemo(() => new Date(), []);

  const formatFormValues = useCallback((values: Partial<DadosDemandaCadastroJuridico>) => {
    const formattedValues = {
      ...values,
    } as DadosDemandaCadastroJuridico;
  ...
  const updateFormValues = useCallback(
    (newValues: Partial<DadosDemandaCadastroJuridico>) => {
      try {
  ...
  const formularioValido = (dados: DadosDemandaCadastroJuridico) => {
    ...
    if (!dados.tribunal) {
      erros.tribunal = 'Órgão/Tribunal é obrigatório';
    }
    ...
    Object.entries(erros).forEach(([campo, mensagem]) => {
      formBag.setError(campo as keyof DadosDemandaCadastroJuridico, { type: 'required', message: mensagem });
    });

    return Object.keys(erros).length === 0;
  };

  const onSubmit = async (dados: DadosDemandaCadastroJuridico) => {
    try {
      formBag.clearErrors();
  ...
  const handleConsultaProcesso = useCallback(async () => {
    try {
      ...
      setDadosConsultaProcesso(dadosProcessoFormatted);

      formBag.setValue('tribunal', '');
          ...
          {isLoadingForm ? (
            ...
            <div className="flex flex-col gap-4">
              ...
              >
                ...
                <DetalhesProcesso 
                  formBag={formBag} 
                  onConsultaProcesso={handleConsultaProcesso}
                  isConsultandoProcesso={isConsultandoProcesso}
                />

                <DetalhesAcao 
                  formBag={formBag}
                  dadosConsultaProcesso={dadosConsultaProcesso}
                />
                ...
                <div className="flex justify-center space-x-2">
                  ...
                  <Button
                    ...
                    {isJustificandoEnvio ? 'Enviando...' : 'Enviar para exceção'}
                  </Button>
                </div>
              </form>
            </div>
