...
const CausaRaizComponent = ({ formBag }: CausaRaizProps) => {
  ...
  const values = {
    causa_raiz: useWatch({ control, name: 'causa_raiz' }),
  }

  const handleCausaRaizChange = (value: string) => {
    setValue('causa_raiz', value);

    clearErrors('causa_raiz');
  };
        ...
        <div>
          ...
          <FieldMessage.Error.Root>
            <Select
              ...
              <SelectTrigger className={formState.errors.causa_raiz ? "border-red-500" : ""}>
            ...
            <FieldMessage.Error.Text visible={!!formState.errors.causa_raiz}>
              {formState.errors.causa_raiz?.message}
