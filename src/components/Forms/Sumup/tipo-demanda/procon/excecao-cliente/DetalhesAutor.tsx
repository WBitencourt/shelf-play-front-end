...
const DetalhesAutorComponent = ({ formBag }: DetalhesAutorProps) => {
  ...
  const { formState, setValue, clearErrors, setError, control } = formBag;
      ...
      <FieldMessage.Error.Root>
        ...
        <div
          className={cn(
            ...
            formState.errors.lista_autores ? 'border border-red-500' : ''
          )}>
          <div>
        ...
        <FieldMessage.Error.Text>
          {formState.errors.lista_autores?.message}
        </FieldMessage.Error.Text>
      </FieldMessage.Error.Root>
    </div>
  );
