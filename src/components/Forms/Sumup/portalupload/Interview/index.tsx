...
interface FormInterviewPortalUploadSumUpProps extends HtmlHTMLAttributes<HTMLFormElement>{
  interviewID: string;
}
...
export function FormInterviewPortalUploadSumUp({ interviewID, ...props }: FormInterviewPortalUploadSumUpProps) {
  ...
  const validateForm = (values: FormValuesFormik) => {
    ...
    if(values.tipoDocumento?.value === 'Judicial' && !values.numeroProcesso) {
      errors.numeroProcesso = 'Campo obrigatório para tipo de documento judicial'
    }
  ...
  description: 'Verifique os campos obrigatórios',
      ...
      if(!isWorkTime) {
        toast.warning({
  title: 'Aviso',
  description: 'Aviso! Upload realizado fora do horário comercial, o prazo será contado a partir da próxima hora útil das 09h00 as 17h59.',
});
      }
  ...
  useEffect(() => {
    const loadPage = async () => {
      try {
        ...
        console.info('dadosCliente', dadosCliente);
