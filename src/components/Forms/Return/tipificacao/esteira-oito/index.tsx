...
export interface FormTipificacaoReturnEsteiraOitoParams {
  id: string;
  esteira: string;
  esteiraTipo: string;
...
interface FormTipificacaoReturnEsteiraOitoProps {
  params: FormTipificacaoReturnEsteiraOitoParams;
}
...
export function FormTipificacaoReturnEsteiraOito({ params }: FormTipificacaoReturnEsteiraOitoProps) {
  const pk = params.id;
  ...
  const onSubmit = async (dados: DadosTipificacao) => {
    try {
      ...
      const response = await submeterTipificacao({
        pk,
        dados,
      });
  ...
  const atualizaFormDadosIniciais = useCallback(async () => {
    try {
      ...
      updateDadosCliente({
        timeOut: dadosCliente?.time_out?.['TipificacaoOito'],
