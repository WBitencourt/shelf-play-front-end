...
export interface FormDemandaOutroReturnConsultaDemandaParams {
  id: string;
  esteira: string;
  esteiraTipo: string;
  dadosIniciais: DemandaOutroReturn;
}

interface FormDemandaOutroReturnConsultaDemandaProps {
  params: FormDemandaOutroReturnConsultaDemandaParams;
}
...
export function FormDemandaOutroReturnConsultaDemanda({ params }: FormDemandaOutroReturnConsultaDemandaProps) {
  const pk = params.id;
  ...
  const formBag = useForm<DemandaOutroReturn>();
  ...
  const formatFormValues = useCallback((values: Partial<DemandaOutroReturn>) => {
    const formattedValues = {
      ...values,
    } as DemandaOutroReturn;
  ...
  const updateFormValues = useCallback((newValues: Partial<DemandaOutroReturn>) => {
    try {
  ...
  const onSubmit = async (dados: DemandaOutroReturn) => {
    try {
