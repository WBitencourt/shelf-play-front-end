...
export function BasicInfoForm({ 
  ...
  const { register, setValue, getValues, clearErrors } = formBag;
        ...
        <FieldMessage.Error.Root>
          <div className='flex flex-row gap-2'>
            <Input
              ...
              onChange={(e) => {
                setValue('documento_autor', maskCpfCnpj(e.target.value))
                clearErrors('documento_autor');
              }}
              placeholder="Documento Autor"
              className={cn('w-full', formBag.formState.errors.documento_autor ? 'border-red-500' : '')}
            />
            <Button type="button" onClick={onClickConsultarAutor}>
              {
                isConsultandoAutor ? (
                  'Consultando...'
                ) : (
          ...
          <FieldMessage.Error.Text>
            {formBag.formState.errors.documento_autor?.message}
          </FieldMessage.Error.Text>
        </FieldMessage.Error.Root>
      </div>
        ...
        <div className="flex flex-1 flex-col space-y-2">
          ...
          <FieldMessage.Error.Root>
            <Input 
              ...
              className={formBag.formState.errors.nome_autor ? 'border-red-500' : ''}
              onChange={(e) => {
                setValue('nome_autor', e.target.value);
                clearErrors('nome_autor');
              }}
            />
            <FieldMessage.Error.Text>
              {formBag.formState.errors.nome_autor?.message}
            </FieldMessage.Error.Text>
          </FieldMessage.Error.Root>
        </div>
      </div>
