...
export function FormCadastroJuridicoReturnExcecaoDemandas({ params }: FormCadastroJuridicoReturnExcecaoDemandasProps) {
  ...
  const [observacaoDialogInfo, setObservacaoDialogInfo] = useState<ObservacaoInfo>({
    observacao: {
      value: '',
      isValid: false,
    }
  ...
  const updateDialogObservacaoInfo = (newState: Partial<ObservacaoInfo>) => {
    ...
    setObservacaoDialogInfo((prevState) => {
      if (!prevState) {
        return {
          observacao: {
            value: '',
  ...
  const carregarArquivosDemanda = useCallback(async (files: ArquivoDemanda[] | undefined) => {
    try {
      ...
      toast.error({
        ...
        description: error?.message || 'Falha ao carregar o arquivo. Por favor, tente novamente.',
      });
    } finally {
  ...
  const updateFormValues = useCallback((newValues: Partial<DemandaCadastroJuridicoReturn>) => {
    try {
      ...
      toast.error({
        title: 'Erro ao atualizar os dados do formulário',
  ...
  const handleUpdateContrato = async ({
    ...
    try {
      ...
      const responseGetFillData = await getDadosDemandaExcecao({ pk });
      ...
      toast.error({
        title: 'Erro ao selecionar contrato',
        description: error?.message,
      });
    } finally {
  ...
  const handleConsultarAutor = async () => {
    try {
      ...
      toast.success({
        title: 'Consultado',
        description: 'Autor consultado com sucesso.'
      });
    } catch (error: any) {
      toast.error({
        title: 'Erro ao consultar autor',
        description: 'Não foi possível consultar o autor. Por favor, tente novamente.'
      });
    } finally {
  ...
  const inativar = async () => {
    try {
      ...
      toast.success({
        title: 'Demanda inativada',
        description: 'Demanda inativada com sucesso.',
      });
    } catch (error: any) { 
      toast.error({
        title: 'Falha ao inativar demanda',
        description: error?.message,
      });
    } finally {
  ...
  const adicionaObservacao = async () => {
    try {
      ...
      const response = await adicionaObservacaoDemanda({
        pk,
        observacao: observacaoDialogInfo.observacao.value,
      ...
      updateFormValues({
        observacao: [
          ...oldObservacoes,
          {
            mensagem: observacaoDialogInfo.observacao.value,
        ...
        >
          ...
          <FieldMessage.Error.Root>  
            <Textarea 
              ...
              value={observacaoDialogInfo.observacao.value} 
            />
            <FieldMessage.Error.Text   
              visible={!observacaoDialogInfo.observacao.isValid}
            >
          ...
          <DialogFooter>
            ...
            <Button 
              disabled={!observacaoDialogInfo.observacao.isValid}
