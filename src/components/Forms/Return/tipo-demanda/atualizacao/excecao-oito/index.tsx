...
export interface AtualizacaoReturnPageParams {
  id: string;
  esteira: string;
  esteiraTipo: string;
  dadosIniciais?: DemandaAtualizacaoReturn;
}

interface FormAtualizacaoReturnExcecaoDemandasProps {
  params: AtualizacaoReturnPageParams;
}
...
export function FormAtualizacaoReturnExcecaoDemandas({ params }: FormAtualizacaoReturnExcecaoDemandasProps) {
  const pk = params.id;
  ...
  const formBag = useForm<DemandaAtualizacaoReturn>();
  ...
  const [isInativandoDemanda, setIsInativandoDemanda] = useState(false);
  ...
  const [isEnviandoAuditoriaOito, setIsEnviandoAuditoriaOito] = useState(false);
  ...
  const formatFormValues = useCallback((values: Partial<DemandaAtualizacaoReturn>) => {
    const formattedValues = {
      ...values,
    } as DemandaAtualizacaoReturn;

    formattedValues.processo = maskNumeroProcesso(values?.processo);
    formattedValues.prazo_contestacao = values?.prazo_contestacao ? values?.prazo_contestacao?.toString() : '0';
    formattedValues.prazo_liminar = values?.prazo_liminar ? values?.prazo_liminar?.toString() : '0';
  ...
  const updateFormValues = useCallback((newValues: Partial<DemandaAtualizacaoReturn>) => {
    try {
  ...
  const onSubmit = async (dados: DemandaAtualizacaoReturn) => {
    try {
  ...
  const enviarAuditoriaOito = async () => {
    try {
      setIsEnviandoAuditoriaOito(true);
  ...
  description: error?.message,
      ...
      setIsEnviandoAuditoriaOito(false);
    }
  }

  const inativar = async () => {
    try {
      setIsInativandoDemanda(true);
  ...
  const handleClickEnviarDemandaAuditoriaOito = () => {
    setDialogJustificarEnvioOpen(true);

    updateDialogJustificarEnvioInfo({
      tipo: 'enviar-auditoria-oito',
      title: {
        text: 'Justificar envio para auditoria oito',
      },
    });
  }
  ...
  const handleClickInativarDemanda = () => {
    setDialogJustificarEnvioOpen(true);

    updateDialogJustificarEnvioInfo({
      tipo: 'inativar-demanda',
      title: {
        text: 'Justificar o motivo de inativar a demanda',
      },
    });
  }
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <DatePicker
                    ...
                    onSelect={(date) => {
                      formBag.setValue('data_citacao', date?.toISOString())
                    }}
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <DateTimePicker
                    ...
                    onSelect={(date) => {
                      formBag.setValue('data_audiencia', date?.toISOString())
                    }}
                  />
                ...
                <div className="space-y-2">
                  <Label htmlFor="prazo_contestacao">Prazo para apresentar contestação/defesa em dias</Label>
                  <Input
                    id="prazo_contestacao"
                    type="number"
                    disabled
                    {...formBag.register('prazo_contestacao', { required: true })}
                ...
                <div className="space-y-2">
                  ...
                  <DatePicker
                    ...
                    onSelect={(date) => formBag.setValue('data_liminar', date?.toISOString())}
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="prazo_liminar">Prazo para apresentar o cumprimento da liminar/intimação em dias corridos</Label>
                  <Input
                    {...formBag.register('prazo_liminar')}
                    disabled
                    id="prazo_liminar"
                    type="number"
                    placeholder="Prazo liminar"
                  />
                </div>
                ...
                {/* <div className="flex justify-center space-x-2">
                  ...
                  <Button 
                    type="button" 
                    variant='outline'
                    disabled={isEnviandoAuditoriaOito} 
                    onClick={handleClickEnviarDemandaAuditoriaOito}
                  >
                    { isEnviandoAuditoriaOito ? 'Enviando...' : 'Enviar para auditoria oito' }
                  </Button>
                </div>
                ...
                <div className="flex justify-center space-x-2">
                  ...
                  <Button 
                    type="button" 
                    variant="destructive"
                    disabled={isInativandoDemanda} 
                    onClick={handleClickInativarDemanda}
                  >
                    { isInativandoDemanda ? 'Inativando...' : 'Inativar demanda' }
                  </Button>
                </div> */}
              </form>
            )
