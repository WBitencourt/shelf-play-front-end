...
import {
  DemandaProcessoTrabalhistaReturn,
  OnUploadErrorProps,
  FormFile,
  OnDropAccepted,
  OnDropRejected,
...
interface ProcessoTrabalhistaReturnPageParams {
  id: string;
  esteira: string;
  esteiraTipo: string;
  dadosIniciais: DemandaProcessoTrabalhistaReturn;
}

interface FormProcessoTrabalhistaReturnExcecaoDemandasProps {
  params: ProcessoTrabalhistaReturnPageParams;
}
...
export function FormProcessoTrabalhistaReturnExcecaoDemandas({ params }: FormProcessoTrabalhistaReturnExcecaoDemandasProps) {
  const pk = params.id;
  ...
  const formBag = useForm<DemandaProcessoTrabalhistaReturn>();
  ...
  const formatFormValues = useCallback((values: Partial<DemandaProcessoTrabalhistaReturn>) => {
    const formattedValues = {
      ...values,
    } as DemandaProcessoTrabalhistaReturn;
  ...
  const updateFormValues = useCallback((newValues: Partial<DemandaProcessoTrabalhistaReturn>) => {
    try {
  ...
  const onSubmit = async (dados: DemandaProcessoTrabalhistaReturn) => {
    try {
      toast.success({
  title: 'Sucesso',
  description: '',
