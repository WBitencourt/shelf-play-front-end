...
export interface FormOficioPolicialPicPayGroupExcecaoOitoParams {
  id: string;
  esteira: string;
  esteiraTipo: string;
...
interface FormOficioPolicialPicPayGroupExcecaoOitoProps {
  params: FormOficioPolicialPicPayGroupExcecaoOitoParams;
}
...
export function FormOficioPolicialPicPayGroupExcecaoOito({ params }: FormOficioPolicialPicPayGroupExcecaoOitoProps) {
  const pk = params.id;
  ...
  const handleClickConfirmaJustificarEnvio = async (info: JustificativaInfo) => {
    try {
      ...
      toast.error({
        title: 'Erro ao justificar envio',
        description: error?.message
      });
    } finally {
      setIsJustificandoEnvio(false);
