...
export interface FormOficioPolicialPicPayGroupEsteiraOitoParams {
  id: string;
  esteira: string;
  esteiraTipo: string;
...
interface FormOficioPolicialPicPayGroupEsteiraOitoProps {
  params: FormOficioPolicialPicPayGroupEsteiraOitoParams;
}
...
export function FormOficioPolicialPicPayGroupEsteiraOito({ params }: FormOficioPolicialPicPayGroupEsteiraOitoProps) {
  const pk = params.id;
  ...
  const atualizaFormDadosIniciais = useCallback(async () => {
    try {
      ...
      updateDadosCliente({
        timeOut: dadosCliente?.time_out?.['Ofício Policial'] ?? 5,
