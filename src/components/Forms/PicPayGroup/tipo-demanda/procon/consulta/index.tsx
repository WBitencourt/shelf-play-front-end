...
export interface FormProconPicPayGroupConsultaDemandas {
  id: string;
  esteira: string;
  esteiraTipo: string;
  dadosIniciais?: DemandaProconPicPay;
}

interface FormProconPicPayGroupConsultaDemandasProps {
  params: FormProconPicPayGroupConsultaDemandas;
}
...
export function FormProconPicPayGroupConsultaDemandas({ params }: FormProconPicPayGroupConsultaDemandasProps) {
  const pk = params.id;
  ...
  const formBag = useForm<DemandaProconPicPay>();
  ...
  const formatFormValues = useCallback((values: Partial<DemandaProconPicPay>) => {
    const formattedValues = {
      ...values,
    } as DemandaProconPicPay;
  ...
  const updateFormValues = useCallback((newValues: Partial<DemandaProconPicPay>) => {
    try {
  ...
  const onSubmit = async (dados: DemandaProconPicPay) => {
    try {
