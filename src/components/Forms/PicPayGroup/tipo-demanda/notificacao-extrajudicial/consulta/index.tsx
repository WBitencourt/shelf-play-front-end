...
import {
  ...
  ObservacaoInfo,
  JustificativaInfo,
  DemandaNotificacaoExtrajudicialPicPay
} from "../../interfaces";
...
export interface FormNotificacaoExtrajudicialPicPayGroupConsultaDemandas {
  id: string;
  esteira: string;
  esteiraTipo: string;
  dadosIniciais?: DemandaNotificacaoExtrajudicialPicPay;
}

interface FormNotificacaoExtrajudicialPicPayGroupConsultaDemandasProps {
  params: FormNotificacaoExtrajudicialPicPayGroupConsultaDemandas;
}
...
export function FormNotificacaoExtrajudicialPicPayGroupConsultaDemandas({ params }: FormNotificacaoExtrajudicialPicPayGroupConsultaDemandasProps) {
  const pk = params.id;
  ...
  const formBag = useForm<DemandaNotificacaoExtrajudicialPicPay>();
  ...
  const formatFormValues = useCallback((values: Partial<DemandaNotificacaoExtrajudicialPicPay>) => {
    const formattedValues = {
      ...values,
    } as DemandaNotificacaoExtrajudicialPicPay;
  ...
  const updateFormValues = useCallback((newValues: Partial<DemandaNotificacaoExtrajudicialPicPay>) => {
    try {
  ...
  const onSubmit = async (dados: DemandaNotificacaoExtrajudicialPicPay) => {
    try {
      ...
      const response = await submeterDemanda({
        pk,
        dados,
      });
