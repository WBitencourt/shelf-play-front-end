...
import { CircleDollarSign, Trash2, UserRoundCheck } from "lucide-react";
...
export interface FormOficioJudicialNeonEsteiraClientePageParams {
  id: string;
  esteira: string;
  esteiraTipo: string;
...
interface FormOficioJudicialNeonEsteiraClientePageProps {
  params: FormOficioJudicialNeonEsteiraClientePageParams;
}
...
export function FormOficioJudicialNeonEsteiraClientePage({ params }: FormOficioJudicialNeonEsteiraClientePageProps) {
  const pk = params.id;
  ...
  const getDadosCliente = async () => {
    try {
      ...
      toast.error({
        title: 'Falha ao carregar os dados do cliente',
  ...
  const onSubmit = async (dados: DadosDemandaOficioJudicial) => {
    try {
      ...
      toast.error({
        title: 'Fala ao enviar o formulário',
        description: error?.message,
      });
    } finally { 
  ...
  const handleClickConfirmaEnviarExcecao = async () => {
    try {
      ...
      toast.success({
        title: 'Enviado',
        description: 'Exceção enviada com sucesso.',
      });

      router.push(`/${esteira}/${esteiraTipo}`);
    } catch (error: any) {
      toast.error({
        title: 'Falha ao enviar exceção',
        description: error?.message,
      });
    } finally {
  ...
  const handleClickConfirmaAdicionaEmail = async () => {
    try {
      ...
      toast.error({
        title: 'Falha ao adicionar e-mail',
        description: error?.message,
      });
    } finally {
  ...
  const handleClickConfirmaAdicionaParte = async () => {
    try {
      ...
      toast.error({
        title: 'Falha ao adicionar parte',
        description: error?.message,
      });
    } finally {
  ...
  const atualizaFormDadosIniciais = useCallback(async () => {
    try {
      ...
      toast.error({
        title: 'Falha ao carregar os dados do formulário',
        description: error?.message,
      });
    } finally {
        ...
        <CardContent className="flex flex-col gap-4 h-full">
          {
            isLoadingForm ? (
              ...
              >
                          ...
                          <TableRow key={index}>
                            ...
                            <TableCell>
                              <div className="flex justify-center">
                                ...
                                {/* { formatNumberBRLCurrency(parte.saldo.toString()) } */}
                              </div>
                            </TableCell>
                ...
                {/* <EditorText3
                  id="template_resposta_email"
                  label='Template de resposta (E-mail)'
                  value={values.template_resposta_email}
                  onChange={(value) => { formBag.setValue('template_resposta_email', value) }}
                /> */}
