...
export interface FormOficioJudicialNeonEsteiraOitoPageParams {
  id: string;
  esteira: string;
  esteiraTipo: string;
...
interface FormOficioJudicialNeonEsteiraOitoPageProps {
  params: FormOficioJudicialNeonEsteiraOitoPageParams;
}
...
export function FormOficioJudicialNeonEsteiraOitoPage({ params }: FormOficioJudicialNeonEsteiraOitoPageProps) {
  const pk = params.id;
    ...
    try {
      ...
      if (proximaDemanda) {
        toast.success({
          ...
          description: 'Os dados foram submetidos com êxito.',
        });
      ...
      toast.warning({
        title: 'Não há nova demanda para preenchimento',
        description: `Retornando a "${esteira} ${esteiraTipo}"`,
      });
  ...
  const handleConsultaProcesso = async () => {
    try {
      ...
      toast.success({
        title: 'Consulta do processo realizada com sucesso',
        description: 'Os dados foram consultados com êxito.',
      });
    } catch (error: any) {
      toast.error({
        title: 'Erro ao consultar o processo',
        description: error?.message,
      });
    } finally {
  ...
  const handleSelecionarProcesso = async (processoInfo: ProcessoInfo) => {
    try {
      ...
      toast.success({
        title: 'Processo selecionado com sucesso',
        description: 'Os dados do processo foram atualizados.',
      });
    } catch (error: any) {
      toast.error({
        title: 'Erro ao selecionar o processo',
        description: error?.message,
      });
    }
  };
  ...
  const handleProcessUpload = async ({ file, updateFile }: OnProcessUpdateProps) => {
    try {
      ...
      if(isFileSizeExceeded) {
        toast.warning({
          title: 'Aviso de Limite de Arquivo',
  ...
  const handleClickDeleteFile = async ({ file, removeFile, updateFile }: HandleClickDeleteFileProps) => {
    try {
      ...
      toast.error({
        title: 'Erro ao excluir o arquivo',
        description: error?.message,
      });
    } finally {
      updateFile(file.id, {
  ...
  const handleFinalCountDown = async () => {
    try {
      ...
      toast.success({
        ...
        description: 'Demanda desbloqueada com sucesso.',
      });
  ...
  const saveForm = async () => {
    try {
      ...
      toast.success({
        ...
        description: 'Os dados foram salvos com êxito.',
      });
    } catch (error: any) { 
      toast.error({
        title: 'Erro ao salvar o formulário',
  ...
  const adicionaObservacao = async () => {
    try {
      ...
      toast.success({
        ...
        description: 'A observação foi registrada com êxito.',
      });
    } catch (error: any) { 
      toast.error({
  ...
  const handleClickConfirmaAdicionaObservacao = async () => {
    try {
      ...
      toast.error({
        ...
        description: 'A observação deve conter no mínimo 10 caracteres.',
      });
    } finally {
      setIsAddingObsForm(false);
                    ...
                    <TableBody>
                      {
                        values?.partes && values.partes.length > 0 ? (
                          values?.partes?.sort((a, b) => a.nome.localeCompare(b.nome)).map((parte, index) => (
                            <TableRow key={`${parte.nome}-${parte.documento}`}>
                              <TableCell className='w-full'>
                                { parte.nome }
                              </TableCell>
