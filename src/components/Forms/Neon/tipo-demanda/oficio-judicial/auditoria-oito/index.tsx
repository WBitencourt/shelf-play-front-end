...
import {
  aprovarDemanda,
  reprovarDemanda,
  getDadosDemandaAuditoriaOito,
} from "@/actions/backend-for-front-end/auditoria";
...
export interface FormOficioJudicialNeonAuditoriaOitoPageParams {
  id: string;
  esteira: string;
  esteiraTipo: string;
...
interface FormOficioJudicialNeonAuditoriaOitoPageProps {
  params: FormOficioJudicialNeonAuditoriaOitoPageParams;
}
...
export function FormOficioJudicialNeonAuditoriaOitoPage({ params }: FormOficioJudicialNeonAuditoriaOitoPageProps) {
  const pk = params.id;
  ...
  const handleProcessUpload = async ({ file, updateFile }: OnProcessUpdateProps) => {
    try {
      ...
      toast.error({
        title: 'Falha ao enviar o arquivo. Por favor, tente novamente',
        description: error?.message,
      });
    }
  }
  ...
  const enviarExcecaoOito = async () => {
    try {
      ...
      toast.error({
        title: 'Erro ao enviar para exceção oito',
        description: error?.message
      });
    } finally {
  ...
  const handleClickSair = async () => {
    try {
      ...
      toast.error({
        title: 'Erro ao desvincular o usuário',
        description: error?.message
      });
    }
  }
