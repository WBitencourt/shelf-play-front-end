...
export interface FormOficioJudicialNeonExcecaoClientePageParams {
  id: string;
  esteira: string;
  esteiraTipo: string;
...
interface FormOficioJudicialNeonExcecaoClientePageProps {
  params: FormOficioJudicialNeonExcecaoClientePageParams;
}
...
export function FormOficioJudicialNeonExcecaoClientePage({ params }: FormOficioJudicialNeonExcecaoClientePageProps) {
  const pk = params.id;
  ...
  const carregarArquivosDemanda = useCallback(async (files: ArquivoDemanda[] | undefined) => {
    try {
      ...
      toast.error({
        title: 'Falha no carregamento de arquivo',
  ...
  const enviarExcecaoOito = async () => {
    try {
      ...
      toast.error({
        title: 'Erro ao enviar para exceção cliente',
        description: error?.message
      });
    } finally {
      setIsJustificandoEnvio(false);
  ...
  const enviarEsteiraCliente = async () => {
    try {
      ...
      toast.error({
        title: 'Erro ao enviar para esteira cliente',
        description: error?.message
      });
    } finally {
      setIsJustificandoEnvio(false);
  ...
  const enviarEsteiraOito = async () => {
    try {
      ...
      toast.error({
        title: 'Erro ao enviar para esteira oito',
        description: error?.message
      });
    } finally {
      setIsJustificandoEnvio(false);
  ...
  const enviarAuditoriaOito = async () => {
    try {
      ...
      toast.error({
        title: 'Erro ao enviar para auditoria oito',
        description: error?.message
      });
    } finally {
      setIsJustificandoEnvio(false);
  ...
  const substituirEnderecoEmail = async () => {
    try {
      ...
      toast.success({
        title: 'E-mail substituído',
        description: 'E-mail substituído e re-enviado sucesso.'
      });
    } catch (error: any) { 
      toast.error({
        title: 'Erro ao substituir e-mail',
        description: error?.message
      });
    } finally {
      setIsJustificandoEnvio(false);
                          ...
                          <TableRow key={index}>
                            ...
                            <TableCell className='w-full'>
                              <Tooltip.Root>
                                <Tooltip.Trigger>
                                  <Trash2 
                                    className="text-red-500 h-5 w-5 cursor-pointer disabled:cursor-not-allowed" 
