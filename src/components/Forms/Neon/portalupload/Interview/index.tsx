...
interface FormInterviewPortalUploadNeonProps extends HtmlHTMLAttributes<HTMLFormElement>{
  interviewID: string;
}
...
export function FormInterviewPortalUploadNeon({ interviewID, ...props }: FormInterviewPortalUploadNeonProps) {
  ...
  useEffect(() => {
    const loadPage = async () => {
      try {
        ...
        toast.error({
          title: 'Página não carregou corretamente',
          description: error?.message,
        });
      } finally {
